...
class CustomObserverBase(ObserverBase):
    ...
    def set_qmin_qmax(self, quant_min, quant_max):
        if quant_min is None or quant_max is None:
            ...
            # print(self.dtype)
            # print(torch.iinfo(self.dtype).max)
    ...
    def _calculate_qparams(self, min_val, max_val):
        ...
        if self.qscheme == torch.per_tensor_symmetric or self.qscheme == torch.per_channel_symmetric:
            ...
            scale = max_val / (float(self.qmax - self.qmin) / 2)
            ...
            zero_point = torch.max(zero_point, torch.tensor(self.qmin, device=device, dtype=zero_point.dtype))
            zero_point = torch.min(zero_point, torch.tensor(self.qmax, device=device, dtype=zero_point.dtype))
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(CustomPerChannelMinMaxObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict,
                                                                    missing_keys, unexpected_keys, error_msgs)
...
class CustomHistogramObserver(CustomObserverBase):
    r"""
    ...
    def __init__(self, bins=2048, upsample_rate=128, dtype=torch.quint8,
        ...
        super(CustomHistogramObserver, self).__init__(dtype=dtype,
                                                qscheme=qscheme,
                                                reduce_range=reduce_range,
                                                quant_min=quant_min,
                                                quant_max=quant_max)
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        super(CustomHistogramObserver, self)._save_to_state_dict(destination, prefix, keep_vars)
        destination[prefix + 'min_val'] = self.min_val
        destination[prefix + 'max_val'] = self.max_val
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(CustomHistogramObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict,
                                                             missing_keys, unexpected_keys, error_msgs)